...
constexpr int kNumContentTypes = sizeof(kContentTypes) / sizeof(kContentTypes[0]);

std::string contentTypeToStr(const ContentType& contentType)
{
    if(contentType.contentType==0)
    {
        if(contentType.contentTypeStr.empty())
            return "binary/octet-stream";
        return contentType.contentTypeStr;
    }
    if(contentType.contentType > 0 && contentType.contentType <= kNumContentTypes)
        return kContentTypes[contentType.contentType-1];
    assert(false);
    return "unknown/unmapped";
}

folly::F14FastMap<std::string_view, int> makeContentTypeMap()
{
    folly::F14FastMap<std::string_view, int> contentTypeMap;
    for(int i = 0; i < kNumContentTypes; ++i)
    {
        contentTypeMap[kContentTypes[i]] = i+1;
    }
...
{
    if(contentTypeStr == "binary/octet-stream")
        return ContentType{0, ""};

    static folly::F14FastMap<std::string_view, int> contentTypeMap = makeContentTypeMap();
    auto it = contentTypeMap.find(contentTypeStr);
...
void ContentType::serialize(CWData& wdata) const
{
    if(contentType>0)
    {
        wdata.addVarInt(contentType);
    }
    else
    {
        wdata.addVarInt(0);
        wdata.addString2(contentTypeStr);
    }
}

bool ContentType::deserialize(CRData& rdata)
{
    int64_t contentTypeRead;
    if(!rdata.getVarInt(&contentTypeRead))
        return false;
    contentType = static_cast<int>(contentTypeRead);
    if(contentType == 0)
    {
        if(!rdata.getStr2(&contentTypeStr))
            return false;
    }
    else if(contentType < 0 || contentType > kNumContentTypes)
    {
        return false;
    }
    return true;
